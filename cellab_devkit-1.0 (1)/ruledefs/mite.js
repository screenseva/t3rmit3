    ...
    */
    ...
    rule.worldtype = 1;          // 2D torus, 8 neighbours
    rule.patreq = "mite";
    rule.palreq = "mite";
    rule.randb = 1; // Randombit   Random bits in planes 1-3
    rule.randn = 3; // Randombits

    function mite(oldstate,     nw, n  , ne,
                                w, self, e,
                                sw, s  , se) {
        ...
        var newstate, phase, grass, cow, direction, newself, eightsum;

        cow = BITFIELD(oldstate, Cowbit, 1);
        grass = BITFIELD(oldstate, Grassbit, Grassbits);
        phase = BITFIELD(oldstate, Phasebit, Phasebits);

        switch (phase) {
            case 0:             // Update Cows
                if (cow == 0) {
                    direction = BITFIELD(oldstate, Randombit, Randombits);
                    switch (direction) {
                        case 0: cow = nw;
                            break;
                        case 1: cow = n;
