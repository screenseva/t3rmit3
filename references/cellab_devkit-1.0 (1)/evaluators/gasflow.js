    ...
    /*
        Gas flow with wall constraints
        ...
        single call on Math.random() for the percentage test,
        the direction of diffusion, and the flow bias.
        Math.random() is expensive to call, and we'd rather not
        call it three times for every cell we update.  The
        multiply and mod on the subsequent uses avoids
        correlation of direction with whether the cell is to be
        updated.
    */

    //  Strength of flow: 0 (no bias) to 100 (counter-flow prohibited)
    rule.evaluator.flow = 25;
    ...
    /*  The flow will be biased against moving in the directions
        indicated below, which correspond to the neighbours in
        nindex above.  */
    rule.evaluator.nogo = [     true,   false,  false,
                                true,           false,
                                true,   false,  false
                          ];

    function gasflow(cells, phyx, phyy, p, lut, ncells) {
        ...
        /*  If a populated cell and a neighbour are in a "nogo"
            direction (seen in inverse directions from one another,
            skip the update if the random value exceeds the flow
            percentage.  */
        if (rule.evaluator.flow > 0) {
            if ((((self != 0) && (neigh == 0) && rule.evaluator.nogo[r]) ||
                 ((self == 0) && (neigh != 0) && rule.evaluator.nogo[7 - r])) &&
                (Math.floor(((rx * 169457) % 1.0) * 100) < rule.evaluator.flow)) {
                return self;
            }
        }
