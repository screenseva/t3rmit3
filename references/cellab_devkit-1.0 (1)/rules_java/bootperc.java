/*
    ...
    don't forget to adjust density accordingly.) You can
...
class Bootperc extends ruletable {

    /*  Set vonnN true to evaluate in von Neumann
        neighbourhood (4 neighbours), false for Moore
        neighbourhood (8 neighbours).  */
    final static boolean vonnN = false;
    
    final static double density = 8;    // Random seed density percent


    void jcruleModes() {
        setWorld(1);                    // 2D torus world
        setPatternRequest("zero");
        setPaletteRequest("bootperc");
        setInitialRandomSeed(0, 1, (int) Math.round(density * (255.0 / 50)));
    }
...
public class bootperc {
    public static void main(String args[]) {
       (new Bootperc()).generateRuleFile("bootperc");
    }
}
